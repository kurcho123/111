...
local function OpenEntranceMenu()
    ...
    if IsOwned then
        ...
        --     }
        -- }
        table.insert(headerMenu.options, {
     ...
     elseif not IsOwned then
        ...
        --     }
        -- }
        print("Moving apartments disabled...")
     end
    ...
    lib.registerContext(headerMenu)
...
local function RegisterApartmentEntranceTarget(apartmentID, apartmentData)
    ...
    exports.ox_target:addBoxZone({
        name = boxName,
        coords = coords,
        size = vec3(boxData.length, boxData.width, 1.5),
        rotation = 1.5,
        options = options,
    })
...
local function RegisterInApartmentTarget(targetKey, coords, heading, options)
    ...
    local boxName = 'inApartmentTarget_' .. targetKey
    -- exports.ox_target:addBoxZone(boxName, coords, 1.5, 1.5, {
    --     name = boxName,
    --     heading = heading,
    --     minZ = coords.z - 1.0,
    --     maxZ = coords.z + 5.0,
...
local function SetInApartmentTargets()
    ...
    local entrancePos = vector3(Apartments.Locations[ClosestHouse].coords.enter.x + POIOffsets.exit.x, Apartments.Locations[ClosestHouse].coords.enter.y + POIOffsets.exit.y, Apartments.Locations[ClosestHouse].coords.enter.z - CurrentOffset + POIOffsets.exit.z + 1)
...
RegisterNetEvent('apartments:client:OpenStash', function()
    if CurrentApartment ~= nil then
        TriggerServerEvent("apartments:server:LoadPlayerStashes")
        QBCore.Functions.GetPlayerData(function(PlayerData)
            cid = PlayerData.citizenid
            exports.ox_inventory:openInventory("stash", cid .. "-" .. CurrentApartment)
            TriggerServerEvent("InteractSound_SV:PlayOnSource", "StashOpen", 0.4)
            -- exports.ox_inventory:setStashTarget(cid .. "-" .. CurrentApartment, "Apartment Stash")
            -- print(1)
        end) 
    end
end)
