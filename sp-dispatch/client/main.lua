...
local function openMenu()
    print(isJobValid(PlayerData.job.type))
...
local function addBlip(data, blipData)
    ...
    if not alertsMuted and not IsOnDuty() then
        if blipData.sound == "Lose_1st" then
            PlaySound(-1, blipData.sound, blipData.sound2, 0, 0, 1)
        else
            TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 10, blipData.sound or blipData.alert.sound, 0.25)
        end
    end
end
