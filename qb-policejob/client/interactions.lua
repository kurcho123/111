...
RegisterNetEvent('police:client:SearchPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        exports.ox_inventory:RobSearchPlayer()
...
RegisterNetEvent('police:client:JailPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        local input = lib.inputDialog('Изпращане в затвор', {'Време'})
        ...
        if time > 0 then
            TriggerServerEvent("police:server:JailPlayer", playerId, time)
        else
...
RegisterNetEvent('police:client:GetEscorted', function(playerId)
    ...
    QBCore.Functions.GetPlayerData(function(PlayerData)
        print(PlayerData.metadata["isdead"], isHandcuffed, PlayerData.metadata["inlaststand"])
...
RegisterNetEvent("sp-policejob:client:get-cuffed", function(status)
    isHandcuffed = status
end)