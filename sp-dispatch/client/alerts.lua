...
local function PhoneCall(message, anonymous, job, type)
    ...
    if IsCallAllowed(message) then
        ...
        local dispatchData = {
            ...
            codeName = type == '311' and '311call' or type == "511" and "511call" or '911call',
            code = type,
            icon = 'fas fa-phone',
            priority = 2,
            coords = coords,
...
RegisterNetEvent('ps-dispatch:client:sendEmergencyMsg', function(data, type, anonymous)
    local jobs = { ['911'] = { 'leo' }, ['311'] = { 'ems' }, ['511'] = { 'tow' } }
...
-- Custom
local function ChopShop()
    local coords = GetEntityCoords(cache.ped)

    local dispatchData = {
        message = "Кражба на автомобил (Разфасовка)",
        codeName = 'vehicletheft',
        code = '10-90',
        icon = 'fas fa-vehicle',
        priority = 2,
