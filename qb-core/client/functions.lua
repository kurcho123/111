...
--         local ttext = text.text or 'Няма опсиание'
--         local caption = text.caption or 'Нотификация'
...
function QBCore.Functions.Notify(text, notifyType, duration)
    ...
    if type(text) == "table" then
        local title = text.text or 'Notification'
        local description = text.caption or 'Empty notification'
        lib.notify({ 
            id = "qb-notification-"..title,
            title = title, 
