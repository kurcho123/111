...
CreateThread(function()
	for k,v in pairs(PlayerBones) do
        BoneLabelText[v].bid = GetPedBoneIndex(PlayerPedId(), v)
...
function client.openInventory(inv, data)
	...
	if canOpenInventory() then
		...
		if left then
			...
			local PlayerInjuries = exports["sp-ambulance"]:GetPlayerInjuries() or {}
			local PlayerStatus = {
				Head = 100,
				Chest = 100,
				LArm = 100,
			...
			for k, v in pairs(PlayerInjuries) do
				local boneId = nil

				for plyBoneId, plyBone in pairs(BoneLabelText) do
					if plyBone.bid == v.bid then
						boneId = plyBoneId
					end
				end

				if boneId ~= nil then
					PlayerStatus[BoneLabelText[boneId].name] = 100 - (v.count * 10);
				end
			end
			...
			SendNUIMessage({
				...
				data = {
					...
					plyId = PlayerData.source,
					plyName = PlayerData.name,
					plyJob = PlayerInfo.job.label .. " | " .. PlayerInfo.job.grade.name,
					plyNationality = PlayerInfo.charinfo.nationality,
					plyPlayTime = PlayerInfo.playtime,
				}
			})
