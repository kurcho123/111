...
RegisterNetEvent('hospital:client:Revive', function()
    ...
    SendNUIMessage({ action = "updateText", canRevive = false })
end)
...
RegisterNetEvent('hospital:client:SendToBed', function(id, data, isRevive)
    ...
    CreateThread(function()
        Wait(5)
        if isRevive then
            QBCore.Functions.Notify(Lang:t('success.being_helped'), 'success', Config.AIHealTimer * 1000)
...
RegisterNetEvent('qb-ambulancejob:checkin', function()
    if doctorCount >= Config.MinimalDoctors then
        ...
        QBCore.Functions.Progressbar('hospital_checkin', Lang:t('progress.checking_in'), 2000, false, true, {
            ...
            TriggerEvent("sp-ambulance:stop-bleeding-revive")
        end, function() -- Cancel
        ...
        for k, v in pairs(Config.Locations['checking']) do
            ...
            checkingCombo:onPlayerInOut(function(isPointInside)
                if isPointInside then
                    if doctorCount >= Config.MinimalDoctors then
                        lib.showTextUI(Lang:t('text.call_doc'), 'left')
                        CheckInControls('checkin')
                    else
                        lib.showTextUI(Lang:t('text.check_in'), 'left')
                        CheckInControls('checkin')
                    end
                else
                    listen = false
...
exports.ox_target:addBoxZone({
	name = "AjobToggleDuty",
	coords = vec3(352.0, -1404.8, 32.65),
	size = vec3(0.75, 0.65, 0.7),
	options = {
		{
			type = "client",
...
exports.ox_target:addBoxZone({
	name = "clotajob",
	coords = vec3(378.35, -1410.7, 32.95),
	size = vec3(3.25, 3.0, 2.2),
	options = {
		{
			type = "client",
			label = "Съблекалня",
...
exports.ox_target:addBoxZone({
	name = "AmbulanceStash",
	coords = vec3(385.15, -1405.52, 33.85 - 1),
	size = vec3(0.65, 0.4, 0.3),
	options = {
		{
			type = "client",
			label = "Хранилище",
			icon = "fa-solid fa-box",
			event = "sp-ambulance:Storage",
			drawDistance = 3,
			groups = "ambulance",
			distance = 3
		},
...
exports.ox_target:addBoxZone({
	name = "bossajob",
	coords = vec3(352.75, -1428.7, 32.55),
	size = vec3(1.05, 0.45, 0.95),
	options = {
		{
			type = "client",
			label = "Шефско Меню",
...
----------
    ...
    exports.ox_target:addBoxZone({
        name = "террреверре",
        coords = vec3(386.84, -1403.07, 32.9),
        size = vec3(1.2, 0.4, 1.65),
        options = {
            {
                --event = 'wert-ambulancejob:kan-paketle',
                icon = 'fa-solid fa-droplet',
                label = 'Иследване на кръв',
                items = 'bloodtube',
                groups = "ambulance",
                serverEvent = 'sp-ambulancejob:server:kan-paket',
                distance = 2.5,
            },
        },
    })

RegisterNetEvent('sp-ambulance:Storage', function(data, id)
...
RegisterNetEvent('hospital:client:UseIfaks', function()
    local ped = PlayerPedId()
    QBCore.Functions.Progressbar('use_bandage', Lang:t('progress.ifak'), 15000, false, true, {
...
RegisterNetEvent('hospital:client:UseBandage', function()
    local ped = PlayerPedId()
    QBCore.Functions.Progressbar('use_bandage', Lang:t('progress.bandage'), 15000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
...
RegisterNetEvent('hospital:client:UsePainkillers', function()
    ...
    QBCore.Functions.Progressbar('use_bandage', Lang:t('progress.painkillers'), 3000, false, true, {
        ...
        TriggerServerEvent('hud:server:RelieveStress', math.random(40, 45))
    end, function() -- Cancel
        StopAnimTask(ped, 'mp_suicide', 'pill', 1.0)
        QBCore.Functions.Notify(Lang:t('error.canceled'), 'error')
    end)
