...
Config = {}
Config.Debug = false -- \ Задайте стойност true, за да разрешите отстраняването на грешки
Config.Core = "qb-core" -- \ Името на ядрото (по подразбиране: qb-core)
Config.MinimumCops = 0 -- \ Минимален брой полицаи, необходими за продажба на наркотици
Config.GiveBonusOnPolice = true -- \ Дайте бонус за продажба на наркотици на полицията (Редактиране от страна на сървъра)
Config.ChanceSell = 50 -- \  Шанс за продажба на наркотици (в %)
Config.RandomMinSell = 1 -- \ Случайно количество продадени наркотици в диапазона ( min - Минимално количество )
Config.RandomMaxSell = 10 -- \ Диапазон на произволно продаваните количества ( max - Максимално количество )
Config.SellTimeout = 20 -- \ Максимално време, за което можете да изберете опцията си (секунди)
Config.ShouldToggleSelling = false
Config.Zones = { -- \ Зони за продажба (тези зони са свързани с определени наркотици - вижте по-долу)
    ['groove'] = {
        points = {
...
Config.ZoneDrugs = { -- \ Имената трябва да са същите като имената на зоните
-- Към една зона могат да се добавят няколко лекарства, както е показано по-долу
    ["groove"] = {
...
Config.BlacklistPeds = { -- \ Модели Ped, които трябва да бъдат включени в черния списък
    "g_m_m_mexboss_01",
    "cs_bankman",
    "u_m_m_bankman",
    "g_m_m_mexboss_01",
