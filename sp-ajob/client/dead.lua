...
AddEventHandler('gameEventTriggered', function(event, data)
    if event == 'CEventNetworkEntityDamage' then
        ...
        if victimDied and NetworkGetPlayerIndexFromPed(victim) == PlayerId() and IsEntityDead(PlayerPedId()) then
            ...
            local NamePlayerTitttle = GetPlayerName(playerid)
            local playerName = GetPlayerName(playerid) .. ' ' .. '(' .. GetPlayerServerId(playerid) .. ')' or Lang:t('info.self_death')
            local killerId = NetworkGetPlayerIndexFromPed(attacker)
            local killerName = GetPlayerName(killerId) .. ' ' .. '(' .. GetPlayerServerId(killerId) .. ')' or
                                   Lang:t('info.self_death')
            local weaponLabel = (QBCore.Shared.Weapons and QBCore.Shared.Weapons[weapon] and
                                    QBCore.Shared.Weapons[weapon].label) or 'Unknown'
            local weaponName = (QBCore.Shared.Weapons and QBCore.Shared.Weapons[weapon] and
                                   QBCore.Shared.Weapons[weapon].name) or 'Unknown'
            TriggerServerEvent('qb-log:server:CreateLog', 'death',
                '🔸 Играч: ' .. Lang:t('logs.death_log_title2', {TitleName = NamePlayerTitttle}) ..
                "\n🔸 ID: ".. ID ..
                "\n🔸 Стату: " .. "Мъртъв" ..
                "\n🔸 Причина: " .. weaponLabel  .. 
                "\n🔸 Убит от: " .. killerName ..' Уби ' .. playerName .. 
                "\n🔸 Име на оръжието: " .. weaponName
            )
