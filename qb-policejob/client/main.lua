...
RegisterNetEvent("sp-police:client:callsign", function()
    local Player = QBCore.Functions.GetPlayerData()
    local input = lib.inputDialog('Баджов Номер', {
        { type = 'input', label = 'Номер на значка', description = '(Exmaple: 100)', required = true, min = 3, max = 3 },
        { type = 'input', label = 'Име', required = true, disabled = true, default =  Player.charinfo.firstname .. " " .. Player.charinfo.lastname },
    })
 
    if not input then return end

    local badge = tonumber(input[1])
    local name = input[2]

    TriggerServerEvent("sp-policejob:server:callsign", badge, name)
end)
...
local allowedModels = {
    ...
    GetHashKey('nktenf'),
    GetHashKey('nkballer7um'),
    GetHashKey('nkgranger2um'),
    GetHashKey('nkjugularum'),
    GetHashKey('nktenfum'),
    GetHashKey('nkshinobi'),
    GetHashKey('nkcypher'),
    GetHashKey('polcoquette'),
    GetHashKey('polpenumbra'),
    GetHashKey('policet'),
    GetHashKey('policeb'),
}

RegisterNetEvent('sp-policejob:client:fixCar', function()
        ...
        if IsPedInAnyVehicle(ped, false) then
            ...
            if tableContains(allowedModels, currentVehicleHash) then
                if QBCore.Functions.GetPlayerData().job.name == "police" then
                    ...
                    QBCore.Functions.Notify("Не си полицай!", "error")
                end
            else
