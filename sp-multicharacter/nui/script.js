  ...
  window.addEventListener("message", function (event) {
    ...
    if (data.event == "open") {
      ...
      if (characters.length > 0) {
        ...
        $(".container-1 #slider").css("display", "flex").hide().fadeIn(300);
        resolveInfo(0);
        $(".container__info").css("display", "flex").hide().fadeIn(300);
      } else {
        $(".container-1 #slider").hide();
        $(".container__info").hide();
        $(".container__create").css("display", "flex").hide().fadeIn(300);
      }
    }
  });
});

$(document).on("click", ".container-1 #slider #selectcharacter", function (e) {
  ...
  if (selected != char && switching == false) {
    ...
    $.post(
      "https://sp-multicharacter/switch",
      JSON.stringify({ char: selected, citizenid: ele.attr("data-citizenid") }),
      function (data) {
        if (selected >= 0) {
          setTimeout(() => {
...
$(document).on("click", "#createchar_button", function (e) {
  ...
  $.post(
    "https://sp-multicharacter/create",
    JSON.stringify({ char: createdCharacter })
  ).then((res) => {
    if (res == true) {
      $("body").fadeOut(300);
      ...
      $(".container-1 #slider").fadeOut(0);
    } else {
      // TODO: Notify
      console.log("nqma kak");
    }
  });
});
...
$(document).on("click", "#spawnplayer_button", function (e) {
  if (!switching) {
    $.post(
      "https://sp-multicharacter/select",
      JSON.stringify({
        citizenid: $(
          `.container-1 #slider #selectcharacter[data-char='${char}']`
        ).attr("data-citizenid"),
      })
    );
    $("body").fadeOut(500);
    ...
    $(".container-1 #slider").fadeOut(350);
  }
});
...
$(document).on("click", "#deleteplayer_button", function (e) {
  ...
  setTimeout(() => {
    $(".deletechar_modal").css("display", "flex").hide().fadeIn(250);
  }, 200);
});
...
$(document).on("input", "#deleteplayer_input", function (e) {
  if ($(this).val() == characters[char - 1]["name"]) {
    ...
    setTimeout(() => {
      $(".deletechar_modal .preloader-wrapper")
        .css("display", "flex")
        .hide()
        .fadeIn(100);
    }, 50);
    ...
    $.post(
      "https://sp-multicharacter/delete",
      JSON.stringify({
        citizenid: $(
          `.container-1 #slider #selectcharacter[data-char='${char}']`
        ).attr("data-citizenid"),
      ...
      if (res) {
        ...
        $(".deletechar_modal #data").css("display", "flex").hide().fadeIn(0);
        $(".deletechar_modal .preloader-wrapper").fadeOut(0);
      } else {
        $(".deletechar_modal .preloader-wrapper").fadeOut(0);
        setTimeout(() => {
          $(".deletechar_modal #data")
            .css("display", "flex")
            .hide()
            .fadeIn(250);
        }, 50);
...
function createSlider(chars) {
  ...
  if (chars.length > 0) {
    slider += `<a class="waves-effect white black-text" id="selectcharacter" data-char='1' data-citizenid='${
      chars[0].citizenid
    }'>
      <div class="left-info">
        <div class="character-icon"><i class="fa-solid fa-person${
          chars[0]["gender"] != "Male" ? "-dress" : ""
        }"></i></div>
        <div class="character-info">
          <div class="character-info-name">${chars[0].name}</div>
          <div class="character-info-citizenid">${chars[0].citizenid}</div>
        </div>
      </div>
  ...
  for (var i = 1; i < chars.length; i++) {
    slider += `<a class="waves-effect white black-text" id="selectcharacter" data-char='${
      i + 1
    }' data-citizenid='${chars[i].citizenid}'>
      <div class="left-info">
        <div class="character-icon"><i class="fa-solid fa-person${
          chars[0]["gender"] != "Male" ? "-dress" : ""
        }"></i></div>
        <div class="character-info">
          <div class="character-info-name">${chars[i].name}</div>
          <div class="character-info-citizenid">${chars[i].citizenid}</div>
        </div>
      </div>
      <div class="right-info">
        <i class="fa-solid fa-angles-right"></i>
  ...
  if (chars.length < maxchars) {
    slider += `<a class="waves-effect white black-text" id="selectcharacter" data-char='create' data-citizenid='none'>
      <div class="left-info">
        <div class="character-icon">
          <i class="fa-solid fa-question"></i>
        </div>
        <div class="character-info">
          <div class="character-info-name">Нов Герой</div>
        </div>
      </div>
      <div class="right-info">
        <i class="fa-solid fa-angles-right"></i>
  ...
  $(".container-1 #slider").html(slider);
}
