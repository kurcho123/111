...
function LockpickDoor(isAdvanced)
    ...
    local success = exports['sp-minigame']:SkillBar({2000, 3000}, 5, 3) --SkillBar(duration(milliseconds or table{min(milliseconds), max(milliseconds)}), width%(number), rounds(number))
    if success then
    ...
    -- if exports["sp-skillcheck"]:skillCheck("easy") then
    --     Wait(100)
    --     if exports["sp-skillcheck"]:skillCheck("normal") then
    --         Wait(100)
    --         if exports["sp-skillcheck"]:skillCheck("hard") then
    --             LockpickFinishCallback(true)
    --         else
    --             LockpickFinishCallback(false)
    --         end
...
function AttemptPoliceAlert(type, vehicle)
    if not AlertSend then
        ...
        --    TriggerServerEvent('police:server:policeAlert', Lang:t("info.palert") .. type)
            exports['sp-dispatch']:VehicleTheft(vehicle)
        end
        AlertSend = true
...
RegisterNetEvent("qb-vehiclekeys:client:razbiikola", function()
    local success = exports['sp-minigame']:SkillBar({10000, 20000}, 15, 1) --SkillBar(duration(milliseconds or table{min(milliseconds), max(milliseconds)}), width%(number), rounds(number))
    if success then
        local vehicle = QBCore.Functions.GetClosestVehicle()
