...
function ThrowCurrentWeapon()
    ...
    PerformPhysics(prop)
    TriggerServerEvent("sp-drop:throwWeapon", {weapon = weapon, net_id = ObjToNet(prop)})
    throwingWeapon = nil
end
...
RegisterNetEvent("sp-drop:setWeaponData", function(weaponID, data)
                ...
                if NetworkDoesNetworkIdExist(v.net_id) then 
                    ...
                    if dist < 5.0 then 
                        ...
                        if dist < 1.25 and not ShowInteractText(_L("pickup_weapon")) and IsControlJustPressed(1, 51) then 
                            ...
                            Wait(800)
                            TriggerServerEvent("sp-drop:pickupWeapon", k)
                            Wait(800)
                            ClearPedTasks(ped)
